...
# /usr/bin/python3 /Users/tyama/tyama_exp/apps8/test_cd.py
...
group_nodes_list = eppr_obj.get_community_sub_graph_nodes(edge_selfppr, k=30)
               
print("End")
#Gcc, mod_list = eppr_obj.calc_modularity(edge_selfppr, k=110)


#print(f"modularity : {nx.community.modularity(G, part)}")


print(f"commponent num : {len(group_nodes_list)}") # 変更後のグラフ



for i in range(len(group_nodes_list)):
    print(f"conductance {len(group_nodes_list[i])} : {nx.conductance(G, group_nodes_list[i])}")
