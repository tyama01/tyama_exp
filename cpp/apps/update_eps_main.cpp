...
// Files to compile
// c++ update_eps_main.cpp graph.cpp read.cpp -std=gnu++17 -O3 -march=native -o a.out 
...
int main(int argc, char* argv[]){
    ...
    while(flag == 1){
        ...
        min_eps_list.push_back(min_eps); 
    ...
    // 最小のε'を出力
    string result_path = "eps_list.txt";
    ofstream ofs;

    ofs.open(result_path);

    for (int i = 0; i < (min_eps_list.size()); i++){
        ofs << min_eps_list[i] << endl;
    }
